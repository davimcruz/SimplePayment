...
const ViewTransaction: React.FC<ViewTransactionProps> = ({ transactionId }) => {
  ...
  const handleUpdateTransaction = async (formData: ViewTransactionFormData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setApiError(result.error || "Erro ao atualizar a transação")
        toast.error(result.error || "Erro ao atualizar a transação")
      }
    } catch (error) {
      const errorMessage = "Erro ao atualizar a transação. Por favor, tente novamente."
