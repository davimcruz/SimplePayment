...
export async function compararFluxos(userId: number) {
  ...
  const cacheKey = `comparison:${userId}:${anoAtual}`

  try {
    // Verifica cache
    const cachedResult = cache.get<FlowComparison[]>(cacheKey)
    if (cachedResult) {
      return cachedResult
    }
    ...
    console.error('Erro ao comparar fluxos:', error)
    throw new Error("Falha ao comparar os fluxos de caixa.")
  }
}
